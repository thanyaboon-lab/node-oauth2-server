 ...
 */
async function generateAccessToken(client: OAuth2Server.Client, user: OAuth2Server.User, scope:string) {
  const privateKey = await fs.readFile('private_key.pem');
  console.log('🚀 ~ generateAccessToken privateKey:', privateKey)
 ...
 * Generate Access Token.
 */
async function generateRefreshToken(client: OAuth2Server.Client, user: OAuth2Server.User, scope:string) {
  const privateKey = await fs.readFile('private_key.pem');
  console.log('🚀 ~ generateRefreshToken privateKey:', privateKey)
  // Create a payload for the JWT
  const payload = {
    // Standard JWT claims
